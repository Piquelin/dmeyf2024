...
CN_canaritos_asesinos_base <- function( pinputexps, ratio, desvio)
{
  ...
  param_local$train$training <- c( 202102, 202105)
...
KA_evaluate_kaggle_semillerio <- function( pinputexps )
{
  ...
  param_local$envios_hasta <- 1350L
...
c03_SEMI_post_jueves_ranks <- function( pnombrewf )
{
  param_local <- exp_wf_init( pnombrewf ) # linea fija

  # Etapa especificacion dataset de la Segunda Competencia Kaggle
  DT_incorporar_dataset( "~/buckets/b1/datasets/competencia_03_ranks.csv.gz")
  ...
  DC_eliminar_bajas1
  CN_canaritos_asesinos_base(ratio=0.6, desvio=3.0)
